...
# Read in author reference dataframes
small <- read.csv("08_5_med_psbO_authorReference.csv")  %>% 
  rename(size_fraction = Size.fraction..µm.)

mid   <- read.csv("5_20_med_psbO_authorReference.csv") %>% 
  rename(size_fraction = Size.fraction..µm.)

large <- read.csv("20_180_med_psbO_authorReference.csv") %>% 
  rename(size_fraction = Size.fraction..µm.)

# Read in csv matching stations to coords
stations <- read.csv("tara_stations.csv")

# Load in the pie map functions
...
# choose radius_scale to make the pie charts more visible
pie_scale_small <- 5
pie_scale_mid   <- 12
pie_scale_large <- 30
...
# Remove legends from the three panels for clarity
...
# Figure too big to display in R, so read to pdf
pdf("bigplot_test.pdf", width = 14, height = 16)
print(final)
dev.off()
